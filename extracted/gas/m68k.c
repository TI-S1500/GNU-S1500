...
#endif
    break;
    /* clm 09/11/90 - special case for cas2 operation */
    /* where operand is a register within parens */
  case '(':
    if(c2=='a' && c3>='0' && c3<='7') {
      n=3;  /* this is the position in str of the register # */
      ret=ADDR+c3-'0';
      ccp[0][n] = '\0';  /* at the end of this call,str is expected to 
			    terminate in '\0' */
    }
    else
      if(c2=='d' && c3>='0' && c3<='7') {
...
obstack_alloc(&robyn,sizeof(struct m68_incant));
	...
	alt_notend_table['D'] = 1;
	alt_notend_table['#'] = 1;
	alt_notend_table['('] = 1;    /* clm 09/10/90 - needed for the CAS2
					 operation */
}
...
char ***vecP;
{
	switch(**argP) {
	case 'c':
		if (!strcmp(*argP,"coff")) do_1500_coff = TRUE;
		else as_warn("Unknown -%s option ignored",*argP);
		while(**argP)
			(*argP)++;
		break;
