...
/* Changes:                                                        */
/* clm 10/17/90 - fixed the 'L' problem where symbols beginning    */
...
/* and so didn't have the '_' feature.  Fix is to strip off the    */
/* '_' at the end of assembly before writing out the symbols.      */
/* At that point all the symbols information is correct, and       */
/* changing the name has no affect.  Have to be sure not to        */
/* count the '_' as part of the symbol's character count when      */
/* calculating the length of the string table.  Fix is contained   */
/* in function write_object_file.                                  */
/* clm 12/13/90 - added handling for the .ident assembler          */
/* directive.  This creates a special section for comments in      */
/* the object file.  Changes have been made to write_object_file   */
/* and to fixup_coff_symbol_table.                                 */
/* clm 04/05/91 - fixed a problem in write_object_file.  The ouput */
/* buffer wasn't being bzero'd out before stuffing it.  This was   */
/* allowing garbage to remain at the end of the buffer at times    */
/* which made the string table appear to be corrupted.             */
...
/* added this to keep track of version number on write.c - Shawn Islam */
#ident "@(#)write.c	1.6 92/03/12"
...
/* clm 12/11/90 - added support for the new comment section - instead of hard coding
  .comment into the_coff_exec above, keep it separate to prevent unnecessary section
  when there is no need for one */
static struct scnhdr comment_scnhdr =  { ".comment", 0, 0 ,0 ,0 ,0 ,0 ,0 ,0 , 0x200 };
/* for padding purposes, word length in bytes, comments must end on a word boundary. */
...
/* 12/11/90 clm - added comment_siz for new comment section */
unsigned comment_siz;
...
{  
  ...
  /* clm 07/03/91 - to be safe, initialize fsize_next to NULL and before
     do the appropriate checks before trying to access it in write_object_file.
     */
  fsizeP->fsize_next = NULL;
...
write_object_file()
{
   ...
   /* clm 12/11/90 - added support for .comment section */
   commentS * commentP;
...

      ...
      used instead of 128. */
	    ...
	    if (symbolP->coff_sy_nlist.n_numaux)
		 ...
		 if (strlen(symbolP->sy_name) > SYMNMLEN) {
		   /* clm 10/17/90 - part of fix for 'L' problem (see
		    change history above) */
		   if (*name == '_' &&
		       L_symbol_p(name)) {
		     /* the symbol name will be one character shorter
			after dropping the '_' */
		     if ( (strlen(symbolP->sy_name) - 1) > SYMNMLEN) {
		       string_byte_count += strlen (symbolP  -> sy_name);
		     }
		   }
		   else
  ...
  the_exec . a_drsize
   ...
   /* clm 12/11/90 - sizeof object file now includes a possible comment
      section. */ 
   if (comment_rootP) {     
     comment_siz = 0;
     /* calculate length of comment section */
     for (commentP = comment_rootP; commentP; commentP = commentP->comment_next) {
       comment_siz += (strlen(commentP->comment) + 1); 
     }
     /* it appears that the comment section needs to be padded to end on a word
	boundary; account for the padding and remember to add padding when writing 
	out comment strings 
	*/
     comment_siz += WORD_LENGTH - (comment_siz % WORD_LENGTH);
     size_of_the_object_file += 
       sizeof(comment_scnhdr) +
	 comment_siz; 
     /* number of sections in file header changes to show .comment section */
     the_coff_exec.the_filehdr.f_nscns += 1;
   } 
   ...
   /* clm 04/02/91 - the following bzero is needed to prevent garbage from
      being left at the end of the the_object_file.  The problem that occurs
      appears to be extra characters at the end of the string table, but the
      characters are just garbage left from the xmalloc... */
   bzero(the_object_file, size_of_the_object_file);
   ...
   /*
     ...
     */
   /* clm 12/11/90 - many of the numbers below must change because of possible 
      .comment section.
      */
   if (do_1500_coff) {
     /* .text section header */
     ...
     if (comment_rootP) {
       the_coff_exec.text_scn.s_relptr += 
	 sizeof(comment_scnhdr) + comment_siz;
       the_coff_exec.text_scn.s_scnptr += sizeof(comment_scnhdr); 
     }
     ...
     if (comment_rootP) {
       /* fill in size and scnptr and append to file */
       comment_scnhdr.s_size = comment_siz;
       comment_scnhdr.s_scnptr = 
	  the_coff_exec.data_scn.s_scnptr + data_siz;  /* assuming no raw data for .bss */
       append (& next_object_file_charP, (char *)(&comment_scnhdr),
	       (unsigned long)sizeof(comment_scnhdr)); 
     }
   }
   else {
...

   ...
   */
   for (fragP = text_frag_root;  fragP;  fragP = fragP -> fr_next)
   {
	...
	if (fragP->fr_fix) 
	  fsize += fragP->fr_fix;
	/* if we have read in a complete function 
	   maintain a linked list of these sizes which
	   we will later associate with the function symbols */	
	if (fragP->fr_fix == 0 &&
	    fsize) {   /* end of the function */
	  fsize_new(fsize);
	  fsize = 0; /* reset fsize for next function */
	}
	else if ((fragP->fr_symbol == 0 &&
		  fragP->fr_opcode == 0) &&
		 fsize) {   /* also have come to the end of a 
			       function but there are some trailers */
	  if (fsize == fragP->fr_fix) {
	    /* case of a small function fitting in one frag */
	    /* sorry about redundant code, but i'm in a hurry */ 
	    for (bc = 0; bc < (fragP->fr_fix - 1); bc++) {
	      if ((fragP->fr_literal[bc] == 0x4e) &&
		  (fragP->fr_literal[bc+1] == 0x75)) {  /* an rts */
		    fsize = fsize - (fragP->fr_fix - (bc + 2));
		    fcnp = 1;
		    break;
		  }
	    }
	    ...
	    else fsize = 0; /* case where we have something other than a fcn 
			  in the text */
	  }
	  else {  /* we have the end of a function followed immediately
		      by some data.  this means we must search for the
		      end of the function */
	    for (bc = 0; bc < (fragP->fr_fix - 1); bc++) {
	      if ((fragP->fr_literal[bc] == 0x4e) &&
		  (fragP->fr_literal[bc+1] == 0x75)) {  /* an rts */
	    ...
	    /* if not found just leave the size as is - */
	    fsize_new(fsize);
	    fsize  = 0;
	  }
	}
   ...
   if (comment_rootP) {
     ...
     for (commentP = comment_rootP; commentP; commentP = commentP->comment_next) {
       string_length = strlen(commentP->comment) + 1; 
       bytes_out += string_length;
       append (& next_object_file_charP, commentP->comment,
	       (unsigned long)string_length);
     }
     if (comment_siz > bytes_out) {  /* pad */
       int tempi;
       for(tempi = bytes_out; tempi < comment_siz; tempi++)
	 append(&next_object_file_charP, "", (unsigned long)1); 
     }
   }
   /*
    * Emit relocations.
   ...
   */
				...
				symbolP = symbolP -> sy_next   )
   {
      ...
      /* clm 10/17/90 - part of fix for 'L' problem (see change 
	 history above) */
      if (do_1500_coff &&
	  *temp == '_' &&
	  L_symbol_p(temp)) {
	if (!symbolP->coff_sy_nlist. _n . _n_nptr[0] ) {
	  /* i.e., the symbol name is greater than SYMNMLEN (8) chars.
	     Cases: 1) the symbol name is still > SYMNMLEN chars after
	     dropping the '_'; in this case simply drop the char.
	     2) the symbol name is 8 chars after dropping the '_';
	     in this case the symbol will no longer be writen out in
	     the string table, and needs to be put into the name field. */
	  name_length = strlen(symbolP->coff_sy_name);
	  if ((strlen(symbolP->coff_sy_name) - 1) > SYMNMLEN) {
	    temp2 = symbolP->coff_sy_name;
	    strcpy(temp2,temp);
	    temp2++;
	    memcpy(symbolP->coff_sy_nlist. _n . _n_nptr[1], 
		 temp2, name_length );
	  }
	  else {
      ...
      /* clm 12/12/90 - support for .comment section. make sure value of .comment is
	 set to 0; not sure what this value should be, but it doesn't appear to be 
	 used anyway. */
      if (do_1500_coff &&
	  strcmp(temp, ".text") == 0) {
	...
	symbolP->sy_aux->coff_auxent.x_scn.x_nlinno = 0;
      ...
      if (comment_rootP &&
	  strcmp(temp, ".comment") == 0) {
	symbolP->coff_sy_nlist.n_value = symbolP->sy_nlist.n_value = 0;
	symbolP->sy_aux->coff_auxent.x_scn.x_scnlen = 0;
	symbolP->sy_aux->coff_auxent.x_scn.x_nreloc = 0;
	symbolP->sy_aux->coff_auxent.x_scn.x_nlinno = 0;
      }
	...
	/* clm: md_number_to_chars can't handle negative numbers */
	...
	if (!symbolP->coff_sy_nlist.n_sclass ||
	    symbolP->coff_sy_nlist.n_sclass == C_EXT) { 
	  /* either these haven't been handled by a coff 
	     directive, or the sclass has been set to C_EXT 
	     by default. */
	...
	if (symbolP->coff_sy_nlist.n_numaux) {
	  /* have to do number-to-char conversion on all 
	     the fields */
	  ...
	  case bf:
	    ...
	    /* x_endndx */
	    md_number_to_chars((char *)&(symbolP->n_endndx),
			       symbolP->n_endndx,
			       sizeof(symbolP->n_endndx));
	    break;
	  case bb:
	    break;
	  case ef:
	  ...
	  default:
	    ...
	    if ( ISFCN(symbolP->coff_sy_nlist.n_type) ) {
	      ...
	      /* clm 07/03/91 - a problem arose because I wasn't doing a 
		 safety check on fsize_next before the assignment.  The last
		 struct would in some cases point off to garbage. */
	      /* Also need to test for fsizeP if there is no function
		 at all - Shawn Islam				  */
	      if ( fsizeP && fsizeP->fsize_next )
		...
		if (current_ef)
		 /* Check whether there is any static section -- Shawn Islam */
		    symbolP->n_endndx = current_ef->sy_next->sy_number;     
		else
		 /* If no static, point to the beginning of TEXT -- Shawn */
		    symbolP->n_endndx = TEXT_index;
	      md_number_to_chars((char *)&(symbolP->n_endndx),
				 symbolP->n_endndx,
				 sizeof(symbolP->n_endndx)); 
	    }
   ...
   */
   ...
   for ( coff_counter = 0,  symbolP = symbol_rootP;   symbolP;   
     ...
     if (strcmp(symbolP->sy_name, ".ln") == 0)
       /* clm  - need to remove this debug stmt */
   ...
   */
   {
      ...
      do
      {
	    ...
	    /* clm 6/8/90 - added to handle symbols within
	       ...
	       symbol. */
	    /* debug stuff */
	    if (segment_type == SEG_DATA) {
	      if (symbolP) {
		symbolP = NULL;
	      }
	    }
/* trying to fix a core dump-- Shawn Islam */
...
/* unfolding */
                  ...
                  {
#ifdef DONTDEF
                     /* JF these next few lines of code are for the mc68020 which can't 
			handle short
...
int      this_segment_type; /* N_TYPE bits for segment. */
{
   ...
   for ( ;  fixP;  fixP = fixP -> fx_next)
   {
	...
	/* this external refers to a symbol that has 
	   ...
	   created the symbol again.  The fixup
	   informatin should be based on that symbol. */
	for (temp_sym=symbol_rootP;temp_sym;temp_sym=temp_sym->sy_next) {
	  if (strcmp(add_symbolP->sy_name, temp_sym->sy_name) == 0 &&
	      (temp_sym->coff_sy_nlist.n_sclass != C_ARG &&
	       temp_sym->coff_sy_nlist.n_sclass != C_REG &&
	       temp_sym->coff_sy_nlist.n_sclass != C_MOU &&
	       temp_sym->coff_sy_nlist.n_sclass != C_MOS &&
	       temp_sym->coff_sy_nlist.n_sclass != C_FIELD &&
	       temp_sym->coff_sy_nlist.n_sclass != C_MOE &&
	       temp_sym->coff_sy_nlist.n_sclass != C_AUTO &&
	       temp_sym->coff_sy_nlist.n_sclass != C_STRTAG &&
	       temp_sym->coff_sy_nlist.n_sclass != C_UNTAG &&
	       temp_sym->coff_sy_nlist.n_sclass != C_ENTAG)) {
	    add_symbolP = temp_sym;
	    break;
	  }
	}
      }
	...
	/* this external refers to a symbol that has 
	   ...
	   informatin should be based on that symbol. */
	for (temp_sym=symbol_rootP;temp_sym;temp_sym=temp_sym->sy_next) {
	  if (strcmp(sub_symbolP->sy_name, temp_sym->sy_name) == 0 &&
	      (temp_sym->coff_sy_nlist.n_sclass != C_ARG &&
	       temp_sym->coff_sy_nlist.n_sclass != C_REG &&
	       temp_sym->coff_sy_nlist.n_sclass != C_MOU &&
	       temp_sym->coff_sy_nlist.n_sclass != C_MOS &&
      ...
      if (symbolP = fixP -> fx_addsy)
      {
		...
		if (do_1500_coff &&
		  ...
		  /* this external refers to a symbol that has 
		     ...
		     informatin should be based on that symbol. */
		  if (symbolP->sy_name[0] == '_') {
		    /* need to place another '_' back on the 
		       string so that symbol_find will be able to
		       find it */
		    keep_the_underscore = TRUE;
		  }

/*		  symbolP = symbol_find(symbolP->sy_name);  */
		  /* there is the possibility that symbol_find will
		     find the wrong symbol; if it returns an argument
		     or a member of a structure, or anything that 
		     obviously be the  object of relocation, keep looking */
		  /* what we really need here is to drop symbol_find and just
		     do a straight search on the symbol table list */
		  for (temp_sym=symbol_rootP;temp_sym;temp_sym=temp_sym->sy_next) {
		  ...
		  keep_the_underscore = FALSE;
		  if (symbolP->sy_number == 0) {
		    /* if the symbol_number is still 0, we have a strange
		       case where the symbol may be in the symbol table linked
		       list, but not in the symbol hash table.  not sure
		       how we get to such a state yet.  in that event, 
		       search the list for the symbol.  question will
		       probably arise one day, 'what if the symbol is not
		       there either?' */
		    /* this can now be incorporated into the test above */
		    for (temp_sym=symbol_rootP;temp_sym;temp_sym=temp_sym->sy_next) {
		      if (strcmp(symbolP->sy_name, temp_sym->sy_name) == 0) {
			symbolP = temp_sym;
			break;
...
/* This is a function to loop through the symbol table 
...
/* clm 12/12/90 - added support for the new .comment section created
   by the .ident assembler directive */
void 
fixup_coff_symbol_table()
{
  symbolS * symbolP;
  symbolS * last_symbolP;   /* to keep track of the last symbol */
  symbolS * ext_symbol_rootP = NULL;
  symbolS * ext_symbol_lastP = NULL;
  symbolS * temp_sym = NULL;
  int      sclass;
  ...
  for (symbolP=symbol_rootP;symbolP;symbolP=symbolP->sy_next) {
	...
	/* before we link this symbol to the externals, make sure 
	   this isn't a reference to a symbol that has been 
	   defined  in the file with a .def directive.
	   this means must loop through the symbol list yet another
	   time.  */
	/* also must make sure that this isn't the definition that will 
	   be followed by global references, in that case we don't want
	   the global refernce, we want this */ 
	/* i.e., if this symbol has been defined by a coff directive, we 
	   know it's not a forward ref */
	...
	if (sclass != C_EXT &&
	  ...
	  for (temp_sym=symbolP->sy_next;temp_sym;temp_sym=temp_sym->sy_next) {
	    int temp_sclass = temp_sym->coff_sy_nlist.n_sclass;
	    if (strcmp(symbolP->sy_name, temp_sym->sy_name) == 0 &&
		/* we may need the same strong check as occurs now in operand 
		   in file expr.c */
		temp_sclass != C_STRTAG  &&
		temp_sclass != C_ENTAG   &&
		temp_sclass != C_UNTAG   &&
		temp_sclass != C_ARG  &&
		temp_sclass != C_REG  &&
	  ...
	  /* look back, if not found */

	  for (temp_sym=symbol_rootP->sy_next;
	       temp_sym != last_symbolP->sy_next;
	       temp_sym=temp_sym->sy_next) {
	    int temp_sclass = temp_sym->coff_sy_nlist.n_sclass;
	    if (strcmp(symbolP->sy_name, temp_sym->sy_name) == 0 &&
		/* we may need the same strong check as occurs now in operand 
		   in file expr.c */
	...
	/* even if not a forward ref, this maybe a global ref to a 
	   global symbol already defined by a coff directive.  in that
	   case, must check our external symbol chain. */
	if (sclass == 0 &&
	    forward_ref == 0 &&
	    ext_symbol_rootP) {
	  for (temp_sym=ext_symbol_rootP;temp_sym;temp_sym=temp_sym->sy_next) {
	    if (strcmp(symbolP->sy_name, temp_sym->sy_name) == 0) {
	      forward_ref = 1;
	      break;
	    }
	...
	/* else it is a forward ref and we don't link */
	/* else symbolP = last_symbolP; */
      }  /* if it's an external */
