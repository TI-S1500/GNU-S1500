...
#ifdef ti1500
/* added to compile with X11 R4 -- Shawn Islam */
/* #define XAW_BC   */
#define ASCII_DISK          1
#endif
...
#ifdef ti1500
/* added to compile with X11 R4 on TI S1500 -- Shawn Islam   */
      Widget		src;
#else
      XtTextSource	src;
#endif
...
#ifndef ti1500
/* This library routine is not available on TI X11 for some un Godly
   reason -- Shawn */
...
{
#ifdef ti1500
/* added to compile with TI X11 R4 -- Shawn Islam */
  long start, finish;
#else
...
*/
#ifndef ti1500
/* This function is not available on TI S1500 X11 for some un Godly
   reason -- Shawn Islam  */
  sink = XtAsciiSinkCreate(parent, NULL, 0);
#endif
  
  XtSetArg (fileArgs[0], XtNtextOptions, scrollVertical);
  XtSetArg (fileArgs[1], XtNfile, filename);
/*  XtSetArg (fileArgs[1], XtNtextSource, src); */
#ifndef ti1500
/* This function is not available on TI S1500 X11 for some un Godly
   reason, the sink cannot be created -- Shawn Islam  */
  XtSetArg (fileArgs[2], XtNtextSink, sink);
#endif

#ifdef ti1500
/* The textWidgetClass cannot be created on TI S1500 because the sink
   function is not available on TI X11. So, I am creating asciiDisk
   WidgetClass to display the text from GDB */
  return XtCreateManagedWidget("disk", asciiDiskWidgetClass, parent, fileArgs, XtNumber (fileArgs));
#else
