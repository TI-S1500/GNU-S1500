...
/* Parameters for execution on a ti1500, for GDB, the GNU debugger.
   many of the parameters came from m-news800.h. 

Here is an m-ti1500.h file for gdb version 2.6.  It supports the 68881
registers.
...
{ \
  ...
  if (val < 0)							\
    perror_with_name (NAME);					\
  if (coffhdr.f_magic != TIMAGIC)				\
    error ("File \"%s\" not in pos 7 coff executable format.", NAME);	\
  if (N_BADMAG (hdr))						\
    error ("File \"%s\" not in POS 8 executable format.", NAME); }
#undef READ_FILE_HEADERS(DESC, NAME)
...
#ifndef KERNEL_U_ADDR 
#define XTOB(x) (ctob(x)) 
#define KERNEL_U_ADDR ((unsigned )((0xfffffffe - XTOB(USIZE)) + 2)) 
	/* got this info from /src/cmd/sdb/com/machdep.h  */
#endif

#ifndef NBPG
#define NBPG NBPC  /* number of bytes per page = number of bytes per click */
#endif

#ifndef UPAGES
#define UPAGES USIZE  /* number of pages of u struct */
#endif

/* Address of end of stack space.  */

#define STACK_END_ADDR	(0x07fffffc)
...
#define TI_PS_REGNUM 15
#define TI_PC_REGNUM 16

#define REGISTER_U_ADDR(addr, blockend, regno, fp_end)		\
{	if (regno < SP_REGNUM) \
	  addr = blockend + (regno * 4); \
	else if (regno == SP_REGNUM) \
	  addr = blockend - 4; \
	else if (regno == PS_REGNUM) \
	  addr = blockend + (TI_PS_REGNUM * 4); \
	else if (regno == PC_REGNUM) \
	  addr = blockend + (TI_PC_REGNUM * 4); \
	else if (regno < FPC_REGNUM ) \
	  addr = fp_end + 12 + ((regno - FPC_REGNUM) * 12); \
	else \
	  addr = fp_end + ((regno - FPC_REGNUM) * 4 ); \
}
