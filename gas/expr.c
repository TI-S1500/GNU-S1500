 ...
 */
  ...
  } else if ( is_name_beginner(c) ) /* here if did not begin with a digit */
    {
      ...
      /* If we have a symbol generated by a coff directive for
       * a struct, enum or union declaration, we don't use that.  
       * That causes relocations to the structure itself, not to
       * the external reference to it.  clm 6/15/90
       */
      if (do_1500_coff &&
	  (symbolP->coff_sy_nlist.n_sclass == C_STRTAG ||
	   symbolP->coff_sy_nlist.n_sclass == C_ENTAG ||
	   symbolP->coff_sy_nlist.n_sclass == C_UNTAG ||  
	   symbolP->coff_sy_nlist.n_sclass == C_ARG ||
	   symbolP->coff_sy_nlist.n_sclass == C_AUTO ||
	   symbolP->coff_sy_nlist.n_sclass == C_REG ||
	   symbolP->coff_sy_nlist.n_sclass == C_MOS ||
	   symbolP->coff_sy_nlist.n_sclass == C_FIELD ||
	   symbolP->coff_sy_nlist.n_sclass == C_MOE ||
	   symbolP->coff_sy_nlist.n_sclass == C_MOU)) {  
	/* there must be a subset of this where this applies 
	   probably a combination of C_EXT and C_STAT */
	symbolP = NULL;
      }
	  
      /* clm - if the symbol we just found has as its value
	 the relocatable address of another symbol, then we 
	 do not want it.  this has been shown by a scope problem
	 in PH/lang/c36.c.  we are really looking for the symbol
	 that has as its value the relocatable address of itself */
/*      if (do_1500_coff &&
	  symbolP &&
