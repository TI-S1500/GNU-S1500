   ...
   "cpp %{nostdinc} %{C} %{v} %{D*} %{U*} %{I*} %{M*} %{i*} %{trigraphs} -undef \
    ...
    %{!M*:%{!E:cc1 %{!pipe:%g.cpp} %1 \
		   ...
		   %{S:%{o*}%{!o*:-o %b.s}}%{!S:-o %{|!pipe:%g.s}} |\n\
              %{!S:as %{coff} %{R} %{j} %{J} %{h} %{d2} %a %{gg:-G %g.sym}\
  ...
  for (i = 0; i < n_commands; i++)
    {
      ...
      char *string1 = commands[i].argv[0];

      commands[i].pid = pexecute ((string1 != commands[i].prog ? execv : execvp),
				  string1, commands[i].argv,
				  i + 1 < n_commands);

      if (string1 != commands[i].prog)
	free (string1);
    }
