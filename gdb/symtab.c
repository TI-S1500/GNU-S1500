...
#ifdef ti1500
extern int return_from_func;
extern int return_to_func_addr;
#endif
...
#ifdef ti1500
  /* search for function name first    */
  ...
  for (i=0;i< BLOCKVECTOR_NBLOCKS (bv); i++)
    {
      ...
      if ((BLOCK_START (b) <= pc)
	  && (BLOCK_END (b) > pc) && (BLOCK_START(b) != 0))
             found = 1;	
      if (found)
         break;
     }
  ...
  for (; s && BLOCKVECTOR (s) == bv; s = s->next)
      ...
      if (prev_line >= 0 && prev_pc > best_pc)
	{
	  ...
	  if (i < len)
	    ...
	    best_end = 0;
#ifdef ti1500
         if (best_end && best_pc != pc)
           {
            best_line = item->line;
            b_block = block_for_pc(pc);
        /* if we are returning from a function */
            if (b_block) 
               if ((item->pc >= b_block->endaddr) || return_from_func )
                  {
                    best_line = prev_line;
                   if (pc > return_to_func_addr)
                    return_from_func = 0;
                  }
            }
#endif
...
#ifdef ti1500
  return dummy ? l->item[index].pc : 0;
#else
     ...
     CORE_ADDR *startptr, *endptr;
{
  ...
  /* clm 02/21/91 - the test originally only check index, however, if
     the first function in the file was an exact match, the index would
     be zero, and so zero would be passed back, indicating no match was
     found.  Solution: check exact_match if index is zero.  */
#ifdef ti1500
  if (index ||
      exact_match)
#else
