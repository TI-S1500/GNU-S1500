...
/* 09/17/90 clm - fixed problem causing core dump when trying to print
   doubles using format options.  Problem arose because code was using
   unpack_long on a double, causing an overflow condition with doubles
   larger than LONG_MAX.  Changed following function to not use unpack_long
   on doubles, and to signal an error if the user should try to print 
   a double using any of the integer format args.
*/   
void
     ...
     FILE *stream;
{
  ...
  if (len == sizeof(double))
    val_double = 1;
  else val_long = unpack_long (type, valaddr);
  ...
  switch (format)
	...
	case 0:		/* no size specified, like in print */
	case 'w':
	  if (val_double)  /* clm 09/17/90 */
	    error ("Error: trying to print a double in an integer format\n");
	  else printf ("0x%08x", val_long);
	  break;
	case 'g':
    ...
    case 'a':
      if (val_double)
	error ("Error: trying to print a double in an integer format\n");
      else print_address ((CORE_ADDR) val_long, stream);
      break;
