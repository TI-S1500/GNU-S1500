     ...
     int args;
{
  ...
  if (func)
	...
	funname = misc_function_vector[misc_index].name;
        prev_funname = funname;
       }
      else
       {
          int tryagain = 1;
          int too_much_try = 0;
         if (fi->pc == misc_function_vector[misc_index].address)
            {
             funname = misc_function_vector[misc_index].name;
             prev_funname = funname;
             tryagain = 0;
         ...
         while (tryagain)
          {
             ...
             misc_index = find_pc_misc_function(fi->pc);
             too_much_try = too_much_try + 1;
             funname = misc_function_vector[misc_index].name;
             if (prev_funname != funname)
               {
                 tryagain = 0;
                 prev_funname = funname;
               }
              else if (too_much_try > 9)
                {
                   tryagain = 0; 
                   prev_funname = funname;
                }
